...
class TaskIntegrationSpec extends Specification {
	...
	void "adding TimePlannings to a Task subclass"() {
		...
		def tp1 = new TimePlanning(tstamp: new Date().next(), description: "Start nächste Woche!", timeBudgetPlan: 10, startTime: new Date().clearTime()+7)
		def tp2 = new TimePlanning(tstamp: new Date(), description: "Start morgen!", timeBudgetPlan: 20, startTime: new Date().clearTime().next())
		task.addToPlans(tp1)
		task.addToPlans(tp2)
		...
		then: "Task is saved, TimePlannings are saved too and the latest plan is retrieved from task"
		2 == TimePlanning.count
		10 == task.timeBudgetPlan
		10 == taskr.timeBudgetPlan
	}
	...
	def "compare plan to bookings" () {
		...
		def b2 = new Booking(date: new Date(), amount: 8, start: new Date().clearTime()-2, end: new Date().clearTime()-1)
		def b3 = new Booking(date: new Date(), amount: 10, start: new Date().clearTime()-1, end: new Date().clearTime())
		def tp1 = new TimePlanning(tstamp: new Date().next(), description: "Start nächste Woche!", timeBudgetPlan: 30, startTime: new Date().clearTime()+7)
		def tp2 = new TimePlanning(tstamp: new Date(), description: "Start morgen!", timeBudgetPlan: 20, startTime: new Date().clearTime().next())
		task.addToPlans(tp1)
		task.addToPlans(tp2)
		task.addToBookings b1
